...
async function getGatewayAccount(gatewayFetchFn, userAddress) {
    const response = await gatewayFetchFn("GET", `/v1/accounts/account/${userAddress}`);
    if (response.ok) {
...
async function pollForAccount(gatewayFetchFn, userAddress, msaCreationCallbackFn) {
    // MOCK Timeout
...
export async function startSiwf(userAddress, signatureFn, gatewayFetchFn, encodedSiwfSignedRequest = 'bciqe4qoczhftici4dzfvfbel7fo4h4sr5grco3oovwyk6y4ynf44tsi', signUpHandle, signUpEmail, msaCreationCallbackFn) {
    ...
    if (!hasAccount) {
        ...
        // Sign AddDelegation
        const _ignoreForMockAddDelegationSignature = await signatureFn(userAddress, "eip712", "");
        // Sign Handle
        const _ignoreForMockSetHandleSignature = await signatureFn(userAddress, "eip712", "");
        // Sign Graph Key Add
        const _ignoreForMockAddGraphKeySignature = await signatureFn(userAddress, "eip712", "");
        // Sign Recovery Key
        // const _ignoreForMockSetRecoveryKeySignature = await signatureFn(userAddress, "eip712", "");
        const siwfResponse = mockNewUserResponse();
        // Submit to Gateway
        const _ignoreForMockGatewaySiwfResponse = await postGatewaySiwf(gatewayFetchFn, siwfResponse);
